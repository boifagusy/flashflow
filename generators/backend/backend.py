...
class SocialAuthController
{
    ...
    public function callback($provider, Request $request)
    {
        try {
            ...
            // Generate API token for mobile/desktop apps
            $token = $this->generateApiToken($user);
            
            // Redirect to frontend with token
            $redirectUrl = env('SOCIAL_AUTH_REDIRECT', '/dashboard') . '?token=' . $token;
...
class PaymentController
{
    ...
    public function deletePaymentMethod(Request $request, $id): JsonResponse
    {
        ...
        if ($paymentMethod->is_default) {
            return response()->json(['error' => 'Cannot delete default payment method'], 400);
        }
...
""")
        ...
        controller_content = template.render()
        controller_file = self.backend_path / "app" / "Http" / "Controllers" / "PaymentController.php"
...
""")
        ...
        controller_content = template.render()
        controller_file = self.backend_path / "app" / "Http" / "Controllers" / "FileStorageController.php"
